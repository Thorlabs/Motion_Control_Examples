...
using Thorlabs.MotionControl.TCube.PiezoCLI;

namespace TPZ_Console_net_managed
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			if (argc < 1)
			{
				Console.WriteLine("Usage: TPZ_Console_net_managed serial_number [voltage: (0 - 75)]");
				Console.ReadKey();
				return;
			}
			...
			// Get the TPZ001 serial number (e.g. 81000123)
            ...
            // Get available TCube Piezo Motors and check our serial number is correct  - by using the device prefix
            // (i.e. for serial number 81000123, the device prefix is 81)
            List<string> serialNumbers = DeviceManagerCLI.GetDeviceList(TCubePiezo.DevicePrefix);
			if (!serialNumbers.Contains(serialNo))
			{
				// The requested serial number is not a TPZ001 or is not connected
			...
			// Create the device
			TCubePiezo device = TCubePiezo.CreateTCubePiezo(serialNo);
			if (device == null)
			{
				// An error occured
				Console.WriteLine("{0} is not a TCubePiezo", serialNo);
				Console.ReadKey();
				return;
			}
