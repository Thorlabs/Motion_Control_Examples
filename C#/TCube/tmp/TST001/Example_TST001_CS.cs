...
using Thorlabs.MotionControl.TCube.StepperMotorCLI;

namespace TST_Console_net_managed
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			if (argc < 1)
			{
				Console.WriteLine("Usage: TST_Console_net_managed serial_number [position: (0 - 10)] [velocity: (0 - 5)]");
				Console.ReadKey();
				return;
			}
			...
			// Get the TST001 serial number (e.e 80000123)
            ...
            // Get available TCube Stepper Motor and check our serial number is correct - by using the device prefix
            // (i.e. for serial number 80000123, the device prefix is 80)
            List<string> serialNumbers = DeviceManagerCLI.GetDeviceList(TCubeStepper.DevicePrefix);
			if (!serialNumbers.Contains(serialNo))
			{
				// The requested serial number is not a TST001 or is not connected
			...
			// Create the device
			TCubeStepper device = TCubeStepper.CreateTCubeStepper(serialNo);
			if (device == null)
			{
				// An error occured
				Console.WriteLine("{0} is not a TCubeStepper", serialNo);
				Console.ReadKey();
				return;
			}
            ...
            ThorlabsTCubeStepperMotorSettings currentDeviceSettings = device.MotorDeviceSettings as ThorlabsTCubeStepperMotorSettings;
