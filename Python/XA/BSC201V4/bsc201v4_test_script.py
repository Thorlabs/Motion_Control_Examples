...
BSC201V4_PART_NUMBER = "BSC201"
BSC201V4_PARENT_SERIAL = "40003334"
BSC201V4_CHANNEL_SERIAL = "40003335"
DEFAULT_TIMEOUT = 5000

SIMULATION_CONFIG_STRING = {
    "PartNumber": BSC201V4_PART_NUMBER,
    "SerialNumber": BSC201V4_PARENT_SERIAL,
    "Channels": [
        {"BayNumber": 1, "SerialNumber": BSC201V4_CHANNEL_SERIAL, "ActuatorType": "DRV001"},
...
def create_simulation(system_manager: SystemManager) -> TLMC_DeviceInfo | None:
    try:
        ...
        print(f"Unable to create simulation for BSC201V4: {e.error_code}")
        return None

    device_info = TLMC_DeviceInfo()
    device_info.device_type_description = "BSC201V4"
    device_info.device = BSC201V4_CHANNEL_SERIAL
    device_info.transport = BSC201V4_PARENT_SERIAL
    return device_info


def main() -> None:
    device: Bsc201v4 | None = None
    ...
    for listed_device_info in system_manager.get_device_list():
        if listed_device_info.device == BSC201V4_CHANNEL_SERIAL:
    ...
    if not device_info:
        print("No compatible BSC201V4 device available.")
        system_manager.shutdown()
        return

    try:
        device = system_manager.open_device_as(device_info.device, device_info.transport, TLMC_OperatingMode.TLMC_OperatingMode_Default, Bsc201v4)
        ...
        DiagnosticsHelper.console(f"Bow index: {device.get_bow_index(DEFAULT_TIMEOUT).name}")
