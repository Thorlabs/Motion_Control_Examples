...
BSC20X_PARENT_SERIAL = "40003334"
BSC20X_CHANNEL_SERIAL = "40003335"
DEFAULT_TIMEOUT = 5000

SIMULATION_CONFIG_STRING = {
    "PartNumber": BSC20X_PART_NUMBER,
    "SerialNumber": BSC20X_PARENT_SERIAL,
    "Channels": [
        {"BayNumber": 1, "SerialNumber": BSC20X_CHANNEL_SERIAL, "ActuatorType": "DRV001"},
...
def create_simulation(system_manager: SystemManager) -> TLMC_DeviceInfo | None:
    ...
    device_info.device_type_description = "BSC20X Logical Channel"
    device_info.device = BSC20X_CHANNEL_SERIAL
    device_info.transport = BSC20X_PARENT_SERIAL
    return device_info


def main() -> None:
    device: Bsc20xLogicalChannel | None = None
    ...
    for listed_device_info in system_manager.get_device_list():
        if listed_device_info.device == BSC20X_CHANNEL_SERIAL:
    ...
    if not device_info:
        print("No compatible BSC20X logical channel available.")
        system_manager.shutdown()
        return

    try:
        device = system_manager.open_device_as(device_info.device, device_info.transport, TLMC_OperatingMode.TLMC_OperatingMode_Default, Bsc20xLogicalChannel)
