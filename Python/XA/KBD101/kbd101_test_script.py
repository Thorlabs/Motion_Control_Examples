...
KBD101_PART_NUMBER = "KBD101"
DEFAULT_TIMEOUT = 5000

SIMULATION_CONFIG_STRING = {
    "PartNumber": KBD101_PART_NUMBER,
    "SerialNumber": "28000001",
    "ActuatorType": "DDSM100",
}
...
def create_simulation(system_manager: SystemManager) -> TLMC_DeviceInfo | None:
    try:
        ...
        print(f"Unable to create simulation for {KBD101_PART_NUMBER}: {e.error_code}")
        return None

    device_info = TLMC_DeviceInfo()
    device_info.device_type_description = KBD101_PART_NUMBER
...
def main() -> None:
    ...
    for listed_device_info in system_manager.get_device_list():
        if listed_device_info.part_number == KBD101_PART_NUMBER:
    ...
    if not device_info:
        print("No compatible KBD101 device available.")
        system_manager.shutdown()
        return

    try:
        device = system_manager.open_device_as(device_info.device, device_info.transport, TLMC_OperatingMode.TLMC_OperatingMode_Default, Kbd101)
