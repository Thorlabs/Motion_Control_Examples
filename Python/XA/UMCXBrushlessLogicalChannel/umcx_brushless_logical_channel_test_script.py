...
UMCX_BRUSHLESS_CHANNEL_SERIAL_NUMBER = "126000003"
...
SIMULATION_CONFIG_STRING = {
    ...
    "Channels": [
        {
            "BayNumber": 1,
            "SerialNumber": UMCX_BRUSHLESS_CHANNEL_SERIAL_NUMBER,
            "ActuatorType": "DDS220",
        }
    ],
}
...
def create_simulation(system_manager: SystemManager) -> TLMC_DeviceInfo:
    ...
    device_info.device_type_description = "UMCX Brushless Logical Channel"
    device_info.device = UMCX_BRUSHLESS_CHANNEL_SERIAL_NUMBER
...
def main() -> None:
    device: UmcxBrushlessLogicalChannel | None = None
    ...
    if not devices:
        ...
        for listed_device_info in devices:
            ...
            is_known_serial = listed_device_info.device == UMCX_BRUSHLESS_CHANNEL_SERIAL_NUMBER
            is_brushless_description = "brushless" in listed_device_info.device_type_description.lower()

            if is_known_serial or is_brushless_description:
                device_info = listed_device_info

        if not device_info:
            print("No UMCX brushless logical channel devices found, continuing with simulation!")
            device_info = create_simulation(system_manager)
    ...
    try:
        device = system_manager.open_device_as(
            ...
            UmcxBrushlessLogicalChannel,
        )

        DiagnosticsHelper.console("Enabling UMCX brushless logical channel...")
        ...
        if enable_state == TLMC_EnableState.TLMC_Enabled:
            DiagnosticsHelper.console_green("UMCX brushless logical channel enabled.")
        else:
            DiagnosticsHelper.console_error("UMCX brushless logical channel not enabled as expected.")
        ...
        general_move_params.backlash_distance = 100
        ...
        velocity_params.acceleration = 200
        velocity_params.max_velocity = 300
        ...
        move_relative_params.relative_distance = 250
        ...
        if device:
            DiagnosticsHelper.console("Shutting down UMCX brushless logical channel...")
            device.close()
